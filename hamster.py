...
async def thailand(ctx):
    ...
    embed = discord.Embed(title=':flag_th:' +response["country"] +':syringe:', description=datestr.strftime("Last update %A %d/%m/%Y, %H:%M:%S") , color=0x00ff00)
    embed.add_field(name=":thermometer_face:ผู้ป่วยทั้งหมด", value='{:,}'.format(response["cases"]), inline=True)
    embed.add_field(name=":mask:ผู้ป่วยวันนี้", value='{:,}'.format(response["todayCases"]), inline=True)
    embed.add_field(name=":skull:เสียชีวิตทั้งหมด", value='{:,}'.format(response["deaths"]), inline=True)
    embed.add_field(name=":skull_crossbones:เสียชีวิตวันนี้", value='{:,}'.format(response["todayDeaths"]), inline=True)
    embed.add_field(name=":mechanical_arm:รักษาหายทั้งหมด", value='{:,}'.format(response["recovered"]), inline=True)
    embed.add_field(name=":muscle:รักษาหายวันนี้", value='{:,}'.format(response["todayRecovered"]), inline=True)
    embed.add_field(name=":hospital:รักษาตัวในโรงพยาบาล", value='{:,}'.format(response["active"]), inline=True)
    embed.set_thumbnail(url='https://phil.cdc.gov//PHIL_Images/23311/23311_lores.jpg')
